...
abbrev equationResidual_eq (P : Params) (x : Time → Length) (v : Time → Speed)
    (a : Time → Acceleration) :
...
abbrev underdamped_not_overdamped (P : Params) :
    IsUnderdamped P → ¬ IsOverdamped P := by
  intro hUnder hOver
  exact lt_asymm hOver hUnder

abbrev undamped_is_underdamped (P : Params) (hgamma : P.gamma = 0) :
    IsUnderdamped P := by
...
abbrev damping_sub_mass_eq_frequency : SI.dampingDim - SI.massDim = SI.frequencyDim := by
  native_decide

abbrev mass_sub_damping_eq_time : SI.massDim - SI.dampingDim = SI.timeDim := by
  native_decide

abbrev zero_add_zero_eq_zero : ((0 : Dim) + (0 : Dim)) = (0 : Dim) := by
  native_decide
...
abbrev qualityFactor_mul_dampingRatio (P : Params) (hgamma : P.gamma.val ≠ 0) :
...
abbrev relaxationTime_mul_dampingRate (P : Params) (hgamma : P.gamma.val ≠ 0) :
...
/-- `γ = 0` 时阻尼残量退化为无阻尼残量。 -/
abbrev equationResidual_gamma_zero
...
/-- `γ = 0` 时，阻尼方程与无阻尼方程等价。 -/
abbrev equationOfMotion_gamma_zero_iff
    (P : Params) (x : Time → Length) (v : Time → Speed) (a : Time → Acceleration)
    (hgamma : P.gamma = 0) :
...
abbrev energyDissipationLaw_eq
