...
/-- 牛顿第二定律的别名展开：`F = m a`。 -/
abbrev newton_second_law (m : Mass) (a : Acceleration) : F_of m a = m * a := rfl
...
/-- 牛顿第二定律的向量形式展开。 -/
abbrev secondLawVec_eq {n : ℕ} (m : Mass) (a : VecAcceleration n) :
    secondLawVec m a = m * a := rfl

/-- 质量恒定时的动量变化公式：`Δp = m Δv`。 -/
abbrev momentum_change_const_mass (m : Mass) (v₂ v₁ : Speed) :
...
abbrev force_from_velocity_rate_const_mass (m : Mass) (dvdt : Acceleration) :
